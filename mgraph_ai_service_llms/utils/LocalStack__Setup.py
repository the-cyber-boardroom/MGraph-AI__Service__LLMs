...
from mgraph_ai_service_llms.config                       import LOCALSTACK__ENABLED, LOCALSTACK__ENDPOINT_URL, LOCALSTACK__REGION_NAME
...
class LocalStack__Setup(Type_Safe):         # Helper class to setup LocalStack for development
    ...
    def is_localstack_enabled(self) -> bool:
        """Check if LocalStack is enabled via environment or config"""
        env_enabled = get_env("LOCALSTACK_ENABLED", "false").lower() == "true"
        return env_enabled or LOCALSTACK__ENABLED
    ...
    def teardown(self):
        ...
        if self.local_stack:
            # LocalStack cleanup if needed
            pass

        if self.temp_aws_credentials:
            # Restore original credentials if needed
            pass

        return self

    @classmethod
    def setup_for_lambda_handler(cls):
        """Static method to setup LocalStack in lambda handler"""
        if get_env("LOCALSTACK_ENABLED", "false").lower() == "true":
            setup = cls()
            setup.setup()
            return setup
        return None