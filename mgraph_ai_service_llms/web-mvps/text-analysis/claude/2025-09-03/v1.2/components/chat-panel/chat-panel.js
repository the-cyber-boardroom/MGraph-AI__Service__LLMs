...
import { TextFormatter } from '../../../../2025-08-22/v1.0/utils/text-formatter.js';
import { getDefaultSample, getRandomWelcome } from '../sample-texts.js';  // Use v1.2 sample-texts
...
export class ChatPanel extends HTMLElement {
    ...
    // v1.1 Update the addMessage method where system messages are rendered
    addMessage(text, sender = 'user', cacheId = null) {
        ...
        if (sender === 'system') {
            ...
            bubble.innerHTML = `<span class="message-text">${this.parseBasicMarkdown(text)}</span>`;
            ...
            // Add generate answer option for questions (with markdown)
            bubble.innerHTML = `
                <span class="message-text">${this.parseBasicMarkdown(text)}</span>
                <button class="inline-generate-btn" data-question="${this.escapeHtml(text)}">Generate Answer</button>
            `;
        } else {
        ...
        if (isQuestion) {
            ...
            generateBtn?.addEventListener('click', async () => {
                ...
                const contextMessages = this.messages.slice(-5).map(msg =>
                    ...
                    const systemPrompt = `\
You are roleplaying as a business user/persona who has requested text analysis. Based on the context provided, answer the following question from THEIR perspective as if you are the user providing information about your organization or situation.

Context of conversation:
${contextMessages}

Important instructions:
- Answer AS the user/persona, not as an advisor
- Provide realistic, specific details about "your" (the persona's) organization
- Use first person ("We have...", "Our company...", "I need...")
- Make up plausible details that fit the context
- Keep answers concise but informative (2-3 sentences)
- Sound like a real person describing their actual situation, not giving advice`;
    ...
    loadSampleText() {
        const sample = getDefaultSample();  // Always loads GDPR sample
    ...
    // v1.1 Add this method to the ChatPanel class
    parseBasicMarkdown(text) {
        ...
        // Single line breaks to <br> (for lists)
        formatted = formatted.replace(/\n/g, '<br>');

        // Simple list support: lines starting with - or *
        formatted = formatted.replace(/^[\-\*]\s+(.+?)(?=<br>|<\/p>)/gm, '• $1');

        return formatted;
    }

    // v1.1 Update the escapeHtml method to be separate (don't modify it)
    escapeHtml(text) {
        const map = {
            '&': '&amp;',
            '<': '&lt;',
    ...
    // v1.1 Add a format helper for the timestamp method
